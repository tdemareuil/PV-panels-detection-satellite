...
import solar
# import solar_4ch # for model with 4th channel
...
# MODEL_NAME = "mask_rcnn_solar_0010.h5" # for model with 4th channel
...
def predict():
    ...
    # Delete files from preivous runs in inference folder
    ...
    # Option 3: Get image under chosen coordinates through Google API
        ...
        if SIZE:
            SIZE = int(SIZE)
            if SIZE // 200 % 2 == 0: SIZE -= 200 # adjust area size
            if SIZE > 200:
        ...
        try:
            solar.call_google_earthenginesolar_API(
                ...
                output_path=app.config['INFERENCE_DIR'],
                compute_RGB_IMG=True,
                compute_Flux_IMG=False,
                compute_Elevation_IMG=False)
            image = os.path.join(app.config['INFERENCE_DIR'], '1_RGB.tif')
        except:
    ...
    # image = solar_4ch.add_rel_lum(image) # add 4th channel
    ...
    if n_solar != 0:
        solar.display_instances(image, yhat['rois'], yhat['masks'],
                            ...
                            save=os.path.join(app.config['INFERENCE_DIR'], "result.png"))
    else:
        skimage.io.imsave(os.path.join(app.config['INFERENCE_DIR'], "result.png"), image)

    # Save mask in inference folder
    mask = yhat['masks'].astype(int)
    mask = mask.sum(axis=2)
    skimage.io.imsave(os.path.join(app.config['INFERENCE_DIR'], "mask.png"), mask)
...
def predict_batch():
    ...
    for index, coords in enumerate(coordinates):
        try:
            solar.call_google_earthenginesolar_API(
                            ...
                            compute_Flux_IMG=False,
                            compute_Elevation_IMG=False)
            image = os.path.join(app.config['INFERENCE_DIR'],
    ...
    with rio.open(os.path.join(app.config['INFERENCE_DIR'], '1_RGB.tif')) as src:
    ...
    total_surface = 0
    total_confidence = 0
    for index, image in enumerate(sorted(glob.glob(os.path.join(app.config['INFERENCE_DIR'],
                                                                '*.tif')))[0:n_images]):
        # Read the image as a numpy array and resize it
        image = skimage.io.imread(image, plugin='matplotlib')
        # image = solar_4ch.add_rel_lum(image) # add 4th channel
...
if __name__ == '__main__':
    ...
    # inference_config = solar_4ch.InferenceConfig() # for model with 4 channels
